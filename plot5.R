...
# Question 5 asks to plot the Emissions from Motor vehicles in Baltimore
# I take Motor vehicles to mean mean self propelled on road vehicles that does 
...
baltimore_vehicle_emissions <- subset(baltimore, baltimore$SCC %in% on_road_sources$SCC)

# get the sum for each year of emissions
totals <- aggregate(Emissions~year, data=baltimore_vehicle_emissions, sum)
